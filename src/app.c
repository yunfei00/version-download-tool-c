...
typedef struct { AppContext* ctx; int row; } DownloadParam;
...
static void progress_cb(int prog, void* ud) { DownloadParam* dp = (DownloadParam*)ud; PostMessage(dp->ctx->hMainWnd, WM_APP_DOWNLOAD_PROGRESS, (WPARAM)prog, (LPARAM)dp->row); }
static DWORD WINAPI download_thread(LPVOID p) {
    ...
    _mkdir("downloads");
    char path[512]; snprintf(path, sizeof(path), "downloads\\%s", a->file_name);
    char err[256] = {0};
    BOOL ok = github_download_file(a->download_url, path, a->size_bytes, progress_cb, dp, err, sizeof(err));
    if (!ok) app_log(ctx, "%s", err); else app_log(ctx, "Saved to: %s", path);
    PostMessage(ctx->hMainWnd, WM_APP_DOWNLOAD_COMPLETE, (WPARAM)ok, (LPARAM)dp->row);
    free(dp); return 0;
}
...
BOOL app_init(HINSTANCE hInstance, int nCmdShow) { INITCOMMONCONTROLSEX icex={sizeof(icex),ICC_LISTVIEW_CLASSES|ICC_PROGRESS_CLASS}; InitCommonControlsEx(&icex);
    ...
    ZeroMemory(&g_app,sizeof(g_app)); g_app.hInstance=hInstance;
...
static void app_init_list_columns(AppContext* c){ LVCOLUMN col={0}; col.mask=LVCF_TEXT|LVCF_WIDTH|LVCF_SUBITEM; const char*t[]={"No","Version","File Name","Size","Published At","Status"}; int w[]={55,100,300,90,180,120}; for(int i=0;i<6;++i){col.iSubItem=i; col.pszText=(LPSTR)t[i]; col.cx=w[i]; ListView_InsertColumn(c->hListView,i,&col);} }
static void app_layout_controls(AppContext* c,int width,int height){ const int m=12,topH=30,btnW=150,g=8,progressH=22,logH=145; int repoW=width-m*2-btnW*2-g*2-8; MoveWindow(c->hRepoEdit,m,m,repoW,topH,TRUE); MoveWindow(c->hRefreshBtn,m+repoW+g,m,btnW,topH,TRUE); MoveWindow(c->hDownloadBtn,m+repoW+g+btnW+g,m,btnW,topH,TRUE); int listTop=m+topH+10; int listH=height-listTop-m-progressH-8-logH-8; MoveWindow(c->hListView,m,listTop,width-m*2,listH,TRUE); int pTop=listTop+listH+8; MoveWindow(c->hProgress,m,pTop,width-m*2,progressH,TRUE); MoveWindow(c->hLogEdit,m,pTop+progressH+8,width-m*2,logH,TRUE);} 

static LRESULT CALLBACK app_wnd_proc(HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam){ AppContext* c=(AppContext*)GetWindowLongPtr(hWnd,GWLP_USERDATA); switch(msg){ case WM_CREATE:{CREATESTRUCT*cs=(CREATESTRUCT*)lParam; c=(AppContext*)cs->lpCreateParams; c->hMainWnd=hWnd; SetWindowLongPtr(hWnd,GWLP_USERDATA,(LONG_PTR)c); app_create_controls(c); SendMessage(c->hProgress,PBM_SETRANGE,0,MAKELPARAM(0,100)); app_log(c,"Application initialized."); return 0;} case WM_SIZE: if(c) app_layout_controls(c,LOWORD(lParam),HIWORD(lParam)); return 0; case WM_COMMAND: if(!c) return 0; switch(LOWORD(wParam)){ case IDC_BTN_REFRESH:{ if(c->refreshing){app_log(c,"Refresh already in progress."); break;} char repo[256]; GetWindowText(c->hRepoEdit,repo,sizeof(repo)); app_clear_releases(c); c->refreshing=TRUE; app_log(c,"Refreshing releases from GitHub..."); RefreshParam* rp=(RefreshParam*)malloc(sizeof(RefreshParam)); rp->ctx=c; strncpy(rp->repo,repo,sizeof(rp->repo)-1); rp->repo[sizeof(rp->repo)-1]='\0'; HANDLE t=CreateThread(NULL,0,refresh_thread,rp,0,NULL); if(t) CloseHandle(t); else { c->refreshing=FALSE; app_log(c,"Failed to create refresh worker thread."); } break;} case IDC_BTN_DOWNLOAD:{ if(c->downloading){app_log(c,"Download already in progress."); break;} int sel=ListView_GetNextItem(c->hListView,-1,LVNI_SELECTED); if(sel<0){app_log(c,"Please select a release asset first."); break;} if(_mkdir("downloads")!=0 && GetLastError()!=ERROR_ALREADY_EXISTS){ app_log(c,"Failed to create downloads directory. error=%lu",GetLastError()); break; } c->downloading=TRUE; ListView_SetItemText(c->hListView,sel,5,"Downloading"); app_log(c,"Start downloading: %s", c->assets[sel].file_name); DownloadParam* dp=(DownloadParam*)malloc(sizeof(DownloadParam)); dp->ctx=c; dp->row=sel; HANDLE t=CreateThread(NULL,0,download_thread,dp,0,NULL); if(t) CloseHandle(t); else { c->downloading=FALSE; app_log(c,"Failed to create download worker thread."); } break;} } return 0; case WM_APP_REFRESH_COMPLETE: if(c){ c->refreshing=FALSE; RefreshResult* rr=(RefreshResult*)lParam; if(rr->success){ c->asset_count=rr->count; for(int i=0;i<rr->count;i++){ c->assets[i]=rr->assets[i]; app_add_release_row(c,i+1,&c->assets[i]); } app_log(c,"Loaded %d release assets.",rr->count);} else app_log(c,"%s",rr->error); free(rr);} return 0; case WM_APP_DOWNLOAD_PROGRESS: if(c){ SendMessage(c->hProgress,PBM_SETPOS,wParam,0); } return 0; case WM_APP_DOWNLOAD_COMPLETE: if(c){ c->downloading=FALSE; int row=(int)lParam; BOOL ok=(BOOL)wParam; ListView_SetItemText(c->hListView,row,5, ok?"Done":"Failed"); app_log(c, ok?"Download completed successfully.":"Download failed."); } return 0; case WM_DESTROY: PostQuitMessage(0); return 0;} return DefWindowProc(hWnd,msg,wParam,lParam);} 
